...
namespace ModelViewer_IntegrationTests
{
	...
	public class MenuItemTest
	{
		...
		public void LaunchCommand()
		{
			...
			{
				CommandID menuItemCmd = new CommandID(FormosatekLtd.ModelViewer.GuidList.guidModelViewerCmdSet, (int)FormosatekLtd.ModelViewer.PkgCmdIDList.cmdidViewModel);

				// Create the DialogBoxListener Thread.
				string expectedDialogBoxText = string.Format(CultureInfo.CurrentCulture, "{0}\n\nInside {1}.MenuItemCallback()", "ModelViewer", "FormosatekLtd.ModelViewer.ModelViewerPackage");
